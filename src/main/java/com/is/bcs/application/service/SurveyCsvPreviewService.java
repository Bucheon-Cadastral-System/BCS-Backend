...
public class SurveyCsvPreviewService implements PreviewSurveyCsvUseCase {
    ...
    @Override
    public SurveyCsvPreviewResult preview(byte[] content, Map<String, String> columnOverrides) {
        SurveyTargetMapper.MappingResult mapped =
                SurveyTargetMapper.map(tableExtractor.extract(content), columnOverrides);

        return new SurveyCsvPreviewResult(
                mapped.totalRows(),
                mapped.columns().recognized(),
                mapped.columns().ignored(),
                SurveyTargetMapper.assignableColumns(),
