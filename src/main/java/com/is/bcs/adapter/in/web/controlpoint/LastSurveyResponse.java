...
/** 기준점의 최종조사 요약. 값이 함께 바뀌므로 한 번에 내려보낸다. */
public record LastSurveyResponse(
    ...
    public static LastSurveyResponse from(LastSurveySummary summary) {
        return new LastSurveyResponse(
                summary.result(), summary.surveyedOn(), summary.surveyorId(), summary.surveyorName(), summary.note());
    }
}
